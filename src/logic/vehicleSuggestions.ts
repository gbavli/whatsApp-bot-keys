...
export class VehicleSuggestionEngine {
   ...
   */
  findBestMakeMatch(userInput: string): string | null {
    ...
    const partialMatch = makes.find(make => 
      make.toLowerCase().startsWith(inputLower) || 
  ...
  getModelsForMake(make: string): string[] {
    const makeModels = this.getAllMakeModels().find(
   ...
   */
  formatModelSelection(make: string): string {
   ...
   */
  getModelByNumber(make: string, selection: number): string | null {
    const models = this.getModelsForMake(make);
   ...
   */
  getYearRangesForVehicle(make: string, model: string): string[] {
   ...
   */
  formatYearRangeSelection(make: string, model: string): string {
