...
function isAmbiguousWord(word, context) {
...
function findBestMake(input, availableMakes, context = []) {
...
function enhancedSimilarityScore(str1, str2) {
...
function findBestModel(input, availableModels, context = []) {
...
function separateMultipleVehicles(input) {
...
function isPhoneNumberPattern(word, context) {
...
function isAddressContext(word, context, index) {
...
function processVehicleString(vehicleString, vehicleData) {
    ...
    if (remainingWords.length === 1) {
        ...
        for (let i = 0; i < remainingWords.length; i++) {
            ...
            if (makeMatch.score > 0.6) {
                ...
                if (modelMatch.score > 0.6) { // Lowered threshold due to better matching
                    ...
                    const dataMatch = vehicleData.find(v => v.make.toLowerCase() === makeMatch.make.toLowerCase() &&
        ...
        for (let makeWords = 1; makeWords <= Math.min(3, remainingWords.length - 1); makeWords++) {
            ...
            if (!makeCandidate || !modelCandidate)
                continue;
